...
class WorkoutController {
  ...
  ValueNotifier<Workout> workout =
      ValueNotifier(Workout(time: null, gymnasticsList: []));
  ...
  void changeDayWorkoutList({required DateTime day}) {
    dayWorkouts.value =
        allUserWorkouts.value!.dayWorkouts[_getOnlyDate(date: day)];
...
// creating new empty workout day and insert in in allUserWorkouts
  ...
  void saveTimeWorkoutBegins({required DateTime time}) {
    ...
    print(
        'saveTimeWorkoutBegins: ${workout.value.time}, ${workout.value.guid}');
  ...
  void addGymnasticsToWorkout({required Gymnastics gymnastics}) async {
    ...
    await myDatabase!.saveWorkout(
        workout: workout.value, userGuid: allUserWorkouts.value!.userGuid);
  }

  void updateExistedWorkoutByGymnastics(
      {required Gymnastics gymnastics}) async {
  ...
  void _updateLocalAllUserWorkouts() {
    final Map<DateTime, List<Workout>?> _mapDaysWorkouts =
        allUserWorkouts.value!.dayWorkouts;
    _mapDaysWorkouts[_getOnlyDate(date: workout.value.time!)] =
        dayWorkouts.value;
...
//      guid: _createNewUuid(),
  ...
  Future<void> loadAndDeserializeData({required String? userGuid}) async {
    ...
    if (allWorkouts != null) {
      Map<DateTime, List<Workout>> _finalMap =
          _mappingAllWorkouts(originalList: allWorkouts);
      allUserWorkouts.value =
          AllUserWorkouts(userGuid: userGuid, dayWorkouts: _finalMap);
    }
  }

  // putting data from firestore to local notifiers
  Map<DateTime, List<Workout>> _mappingAllWorkouts(
      {required List<Workout> originalList}) {
    List<DateTime> _dateWorkoutsList = [];
    originalList.forEach(
        (element) => _dateWorkoutsList.add(_getOnlyDate(date: element.time!)));
    ...
    Map<DateTime, List<Workout>> _map = {
      for (var v in _dateWorkoutsList)
        v: originalList
            .where((element) => _getOnlyDate(date: element.time!) == v)
            .toList()
    };
